...
//  assert("Parsing subvariables...");
  while(lookAhead->tokenType == TK_IDENT)
      compileVarDecl();
//  assert("Subvariables parsed");
}
...
//  assert("Parsing a variable...");
  ...
  // Hỗ trợ khai báo nhiều biến cùng kiểu: x, y, z : INTEGER
...
void compileAssignSt(void) {
  ...
  // Xử lý gán nhiều biến: x,y,z := expr1, expr2, expr3
  while (lookAhead->tokenType == SB_COMMA) {
      eat(SB_COMMA);
      eat(TK_IDENT);
      if (lookAhead->tokenType == SB_LSEL) {
  ...
  // Xử lý các expression tiếp theo cho multi-assignment
  while (lookAhead->tokenType == SB_COMMA) {
      eat(SB_COMMA);
      compileExpression();
  }
