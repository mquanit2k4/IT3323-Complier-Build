...
Token* readString(void) {
  ...
  while (charCodes[currentChar] != CHAR_DOUBLEQUOTE && currentChar != EOF) {
    if (charCodes[currentChar] == CHAR_BACKSLASH) {
      // Store escape sequences as-is (e.g., \n stays as \n)
      token->string[count++] = currentChar;  // Store '\'
      if (count >= MAX_IDENT_LEN) {
        error(ERR_IDENTTOOLONG, token->lineNo, token->colNo);
        break;
      }
      readChar();
      token->string[count++] = currentChar;  // Store the next char (n, t, etc.)
    } else {
      token->string[count++] = currentChar;
    }
  ...
  token->string[count] = '\0';
  readChar(); // Skip closing double quote

  return token;
}

Token* readBytes(void) {
  int count = 0;
  Token* token = makeToken(TK_BYTES, lineNo, colNo);

  // currentChar is already at the opening double quote after 'b' or 'B'
  readChar(); // Skip opening double quote
...
Token* getToken(void) {
  ...
  case CHAR_LETTER:
    // Check for bytes literal (b"..." or B"...")
    if (currentChar == 'b' || currentChar == 'B') {
      ...
      if (charCodes[currentChar] == CHAR_DOUBLEQUOTE) {
        // This is a bytes literal - currentChar is now at "
        return readBytes();
      } else {
        // Not bytes, it's an identifier starting with 'b' or 'B'
        // We need to build the identifier including the already-read character
        token = makeToken(TK_IDENT, ln, cn);
        int count = 0;
        token->string[count++] = firstChar;
...
void printToken(Token *token) {
  ...
  case TK_BYTES: printf("TK_BYTES(b\"%s\")\n", token->string); break;
